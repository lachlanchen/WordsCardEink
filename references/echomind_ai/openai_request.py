...
class OpenAIRequestJSONBase:
    ...
    def text_to_speech(self, text, voice="coral", model="tts-1", instructions="", response_format="mp3", play_audio=True):
        """
        Convert text to speech using OpenAI's TTS API with caching support.
        
        Args:
            text: The text to convert to speech
            voice: Voice to use (alloy, ash, ballad, coral, echo, fable, nova, onyx, sage, shimmer)
            model: TTS model to use (tts-1, tts-1-hd, or gpt-4o-mini-tts if available)
            instructions: Additional instructions for speech generation (only for gpt-4o-mini-tts)
        ...
        while retries < self.max_retries:
            try:
                ...
                # Only add instructions for gpt-4o-mini-tts model
                ...
                if self.use_cache:
                    ...
                    # Create temporary file if not caching
    ...
    def text_to_speech_stream(self, text, voice="coral", model="tts-1", instructions="", response_format="mp3", play_audio=True):
        """
        Convert text to speech with streaming response for faster playback.
        Note: Streaming responses are not cached as they're meant for real-time use.
        ...
        try:
            ...
            error_msg = f"TTS streaming API error: {e}"
            print(error_msg)
            traceback.print_exc()
    ...
    def send_request_with_json_schema(
        self,
        prompt,
        json_schema,
        ...
        while retries < self.max_retries:
            try:
                ...
                if web_search_options is not None:
                    create_kwargs["web_search_options"] = web_search_options
    ...
    def send_simple_request(self, prompt, system_content="You are a helpful AI assistant.", model=None):
        """
        Send a simple text request to OpenAI without JSON schema validation.
