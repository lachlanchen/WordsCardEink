...
class AdvancedWordFetcher:
    ...
    #         print("Pefect string, but cannot be parsed: ", json_string)
    #         raise ValueError(f"Failed to parse JSON: {e}")
...
# class OpenAiChooser:
#     def __init__(self, db, word_fetcher, words_list=words_list):
#         self.db = db
...
class OpenAiChooser:
    ...
    #         if self.original_words_list:
    #             # Restart the iterator from the beginning if the end is reached
    #             self.words_iterator = iter(self.current_words)
    #             return next(self.words_iterator)
    #         else:
    #             # Fetch new words if original_words_list is None
    #             self.fetch_new_words()
    #             return next(self.words_iterator)
